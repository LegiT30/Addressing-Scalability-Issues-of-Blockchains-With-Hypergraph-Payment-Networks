...
**Note:** some generators emit a simplified CSV; we recommend normalizing it to the LN snapshot schema before running the simulator (adds `disabled`, `snapshot_id`, etc.).

### SuperNode
Generate the supernode edges (script should produce `runs/supernode_edges.csv`) and run the simulator:

```bash
# generate supernode clique-style edges (script name may differ in your repo)
python3 scripts/make_supernode_edges.py
python3 run_simulator.py preprocessed runs/supernode_edges.csv params.json outputs/supernode_run
```
### FHS
Generate the FHS edges

```bash
# (optional) normalize CSV to the LN schema if your generator produces simplified columns
python3 scripts/make_fhs_edges_clique.py \
  --m_max 5 \
  --out_prefix runs/fhs_5 \
  --ln_edges ../ln_data/ln_edges.csv \
  --use_edge_capacity \
  --to_clique \
  --max_clique_size 500 \
  --verbose

python3 scripts/fix_fhs_csv.py runs/fhs_5_edges.csv runs/fhs_5_edges_fixed.csv

python3 run_simulator.py preprocessed runs/fhs_5_edges_fixed.csv params.json outputs/fhs_5_run
```
